...
import examplesRefractor.boardgames.allboardgames.Board;
import examplesRefractor.boardgames.allboardgames.TicMove;

public class TicBoard implements Board{
    private TicMove[][] moves;
    private int n;


    public void setUp(int n){
        this.n=n;
        this.moves=new TicMove[this.n][this.n];
        for(int i=0;i<this.n;i++){
            for(int j=0;j<this.n;j++){
                moves[i][j]=TicMove.EMPTY;
            }
        }
    }
    ...
    public String display(){
        ...
        for(int i=0;i<this.n;i++){
            for(int j=0;j<this.n;j++){
                if(moves[i][j]==TicMove.ZERO){
                    sb.append('1');
                }
                else if(moves[i][j]==TicMove.CROSS){
                    sb.append('2');
                }
                else{
                    sb.append('0');
    ...
    public void applyMove(int x, int y, TicMove move){
        if(moves[x][y]!=TicMove.EMPTY) throw new IllegalArgumentException("Invalid move");
        moves[x][y]=move;
    }
